...
function init() {
  ...
  let recorder, stream, chunks = [], elapsed = 0, startedAt = 0, tick, paused = false, audioUrl, latestRecording;
  ...
  elements.start.addEventListener('click', async () => { try { stream = await navigator.mediaDevices.getUserMedia({ audio: true }); const mime = preferredMime(); recorder = mime ? new MediaRecorder(stream, { mimeType: mime }) : new MediaRecorder(stream); chunks = []; recorder.addEventListener('dataavailable', (event) => { if (event.data.size) chunks.push(event.data); }); recorder.addEventListener('stop', async () => { const actualMime = recorder.mimeType || mime || 'audio/webm', blob = new Blob(chunks, { type: actualMime }), filename = `${todayKey}_英语15分钟.${extensionFor(actualMime)}`; if (audioUrl) URL.revokeObjectURL(audioUrl); audioUrl = URL.createObjectURL(blob); latestRecording = { blob, filename, mime: actualMime }; elements.player.src = audioUrl; elements.player.classList.remove('hidden'); await saveAudio({ date: todayKey, filename, mime: actualMime, blob, seconds: elapsed }); meta[todayKey] = { ...(meta[todayKey] || {}), audioFilename: filename, audioFormat: actualMime }; localStorage.setItem(META_KEY, JSON.stringify(meta)); elements.fileInfo.textContent = `${filename} · ${Math.round(blob.size / 1024)}KB · ${actualMime}`; elements.saveBox.classList.remove('hidden'); renderMonthList(); }); recorder.start(); await screenWakeLock.acquire(); startedAt = Date.now() - elapsed * 1000; tick = setInterval(() => { elapsed = Math.floor((Date.now() - startedAt) / 1000); updateTimer(); }, 500); elements.start.disabled = true; elements.pause.disabled = false; elements.stop.disabled = false; elements.status.textContent = '录音中 · 屏幕保持常亮'; } catch (error) { alert(`无法启动麦克风：${error.message || error}`); } });
  elements.pause.addEventListener('click', async () => { if (!recorder) return; if (!paused) { recorder.pause(); clearInterval(tick); await screenWakeLock.release(); paused = true; elements.pause.textContent = '继续'; elements.status.textContent = '录音已暂停'; } else { recorder.resume(); await screenWakeLock.acquire(); startedAt = Date.now() - elapsed * 1000; tick = setInterval(() => { elapsed = Math.floor((Date.now() - startedAt) / 1000); updateTimer(); }, 500); paused = false; elements.pause.textContent = '暂停'; elements.status.textContent = '录音中 · 屏幕保持常亮'; } });
  elements.stop.addEventListener('click', async () => { if (recorder?.state !== 'inactive') recorder.stop(); clearInterval(tick); await screenWakeLock.release(); stream?.getTracks().forEach((track) => track.stop()); elements.start.disabled = false; elements.pause.disabled = true; elements.stop.disabled = true; saveMeta(elapsed >= secondsPerDay); });
